...
$.extend(Datepicker.prototype, {
	...
	_checkOffset: function(inst) {
		...
		var browserWidth = (!$.support.mozilla || document.doctype ?
			document.documentElement.clientWidth : 0) || document.body.clientWidth;
		var browserHeight = (!$.support.mozilla || document.doctype ?
		...
		var above = offset.top - inst.div.outerHeight() -
			(isFixed && $.support.opera ? document.documentElement.scrollTop : 0);
		var below = offset.top + base.outerHeight();
		var alignL = offset.left;
		var alignR = offset.left + base.outerWidth() - inst.div.outerWidth() -
			(isFixed && $.support.opera ? document.documentElement.scrollLeft : 0);
	...
	_doMouseWheel: function(event, delta) {
		...
		if (inst.get('useMouseWheel')) {
			delta = ($.support.opera ? -delta : delta);
			delta = (delta < 0 ? -1 : +1);
	...
	_generateContent: function(target, inst) {
		...
		var picker = this._prepare(renderer.picker, inst).replace(/\{months\}/, monthRows).
			...
			($.support.msie && parseInt($.support.version, 10) < 7 && !inst.inline ?
